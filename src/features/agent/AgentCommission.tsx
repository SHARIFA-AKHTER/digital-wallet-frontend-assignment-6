...
import { useGetAgentCommissionsQuery, useGetUserTransactionsQuery, } from "../transaction/transactions.api";
...
const AgentCommission: React.FC = () => {
  ...
  const totalCommission = commissions.reduce(
    (sum: any, txn: { commission: any; }) => sum + (txn.commission || 0),
    0
  );
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 lg:hidden">
          {commissions.map((txn: { _id: Key | null | undefined; toUser: any; amount: number; commission: any; createdAt: string | number | Date; }) => (
            <div key={txn._id} className="border rounded shadow p-4 bg-white">
              <p><strong>To User:</strong> {txn.toUser || "N/A"}</p>
              <p><strong>Amount:</strong> ৳{txn.amount.toFixed(2)}</p>
              <p><strong>Commission:</strong> ৳{(txn.commission ?? 0).toFixed(2)}</p>
